...
Cloud = function () {
  ...
  var geom = new THREE.BoxGeometry(45, 45, 45, 1, 1, 1);
...
ParticlesHolder.prototype.spawnParticles = function (
  ...
  for (var i = 0; i < nPArticles; i++) {
    ...
    //particle.mesh.position.y = pos.x;
    particle.mesh.position.x = pos.x; // Была ошибка вместо у нужен х
...
CoinsHolder.prototype.spawnCoins = function () {
  var nCoins = 1 + Math.floor(Math.random() * 20);
  var d =
    game.seaRadius +
    game.planeDefaultHeight +
    (-1 + Math.random() * 2) * (game.planeAmpHeight - 20);
...
function updateEnergy() {
  ...
  energyBar.style.backgroundColor = game.energy < 50 ? "#f25356" : "#68c3c0";
...
function updatePlane() {
  ...
  camera.fov = normalize(mousePos.x, -1, 1, 60, 80);
  camera.updateProjectionMatrix();
  camera.position.y +=
  ...
  game.planeCollisionDisplacementY +=
    (0.1 - game.planeCollisionDisplacementY) * deltaTime * 0.03;
  game.planeCollisionDisplacementY +=
...
var fieldDistance, fieldLevel, levelCircle, energyBar, replayMassage;
...
function init(event) {
  ...
  replayMassage = document.getElementById("replayMassage");
